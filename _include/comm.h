...
public:
    ...
    size_t                recvlen;          // 要收到多少数据，由这个变量指定，和precvbuf配套使用，看具体应用的代码
    size_t                recvlen_already;  // 每次发完一个包，用于最后校验收到每个包的长度
    // 发送消息相关
    ...
    size_t          sendlen;          // 当前需要发送的消息长度
    size_t          sendlen_suppose;  // 连接对象应当发送的长度，各连接暂时都为相同值
    size_t          sendlen_already;  // 连接对象已发送长度
    std::atomic_int isend_count;      // 连接对象对应的发送数目

    lp_listening_t  p_listenitem;  // 始终指向监听结构体 m_lplistenitem
    pthread_mutex_t conn_mutex;    // 连接对象互斥量

    time_t inrevy_time;  // 当前连接进入延迟回收队列的时间
    int    inrecyList;   // 表示是否已经在延迟回收队列中，0 表示不在，1 表示在
