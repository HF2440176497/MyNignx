...
public:
    void            ReadConf();
    bool            Initialize();          // master 进程中 g_socket 的初始化工作
...
private:
    ...
    int                               m_worker_connections;     // epoll 连接的最大项数 允许最大连入数
    size_t                            m_total_connection_count; // 连接池中总连接数 m_connectionList.size
    size_t                            m_free_connection_count;  // 连接池中空闲连接对象数 m_free_connectionList.size
    std::atomic_int                   m_online_count;           // 在线用户连接数量
    ...
    size_t                            m_recy_connection_count;  // 延迟待回收的连接个数
    ...
    // 统计相关
    time_t                                                   m_lastprinttime;  // 用于打印信息


public:
    bool    open_listen_sockets();
    void    epoll_event_init();    
    int     epoll_process_events(int timer);
    
public:	

    std::vector<lp_listening_t> m_ListenSocketList; 
};
